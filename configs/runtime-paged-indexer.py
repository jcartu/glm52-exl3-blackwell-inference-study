...
class IndexerPagedMetadata:
    ...
    should set it to the exact indexer-head count they pass to sparkinfer. Replicated
...
def index_topk_fp8(
    *,
    ...
    fold_indices = None
    fold_lengths = None
    fold_mode = os.getenv("SPARKINFER_INDEXER_TWO_LEVEL_FOLD", "auto").strip().lower()
    disabled_fold_modes = {"0", "false", "off", "no"}
    forced_fold_modes = {"1", "true", "on", "yes"}
    if fold_mode not in {"auto", *disabled_fold_modes, *forced_fold_modes}:
        raise ValueError(
            "SPARKINFER_INDEXER_TWO_LEVEL_FOLD must be auto, 0, or 1 "
            f"(got {fold_mode!r})"
        )
    if (
        not output_physical_slots
        and width_tokens >= 2 * _TWO_LEVEL_SLICE_TOKENS
        and fold_mode not in disabled_fold_modes
    ):
        ...
        for c in range(num_chunks):
            ...
            base += splits_c
        total_slices = base
        fold_candidate_nbytes = q_rows * total_slices * topk * 8 + q_rows * 4
        fold_limit_nbytes = (
            int(os.getenv("SPARKINFER_INDEXER_TWO_LEVEL_FOLD_MAX_MIB", "256"))
            * 1024
            * 1024
        )
        if fold_mode == "auto" and fold_candidate_nbytes > fold_limit_nbytes:
            two_level_slices.clear()
            total_slices = 0
        else:
